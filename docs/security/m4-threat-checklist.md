...
| #    | 检查                                                                                                    | 状态 |
| ---- | ------------------------------------------------------------------------------------------------------- | ---- |
| T-13 | 真实设备与浏览器矩阵（Windows Chrome/Edge、Android Chrome、国产浏览器、iOS Safari、微信内置浏览器降级）【2026-08-18 部分验证：Windows Chrome/Edge 真实浏览器生产套件 12/12 + 补充检查 9/9 通过；Android/iOS/微信缺设备未验证，保持未勾选】 | [ ]  |
| T-14 | 真实 Provider 小样本核对（专用低额度 Key、虚构最小样本、条款核对；不可验证的 Provider 诚实标记）【2026-08-18：无专用低额度测试 Key，未执行真实调用；四个 preset 端点本机 TLS 可达性已探针记录于 `docs/release/m4-release-checklist.md`】 | [ ]  |
