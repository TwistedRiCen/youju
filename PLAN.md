...
- M4 Task 1–2 已完成实现与验证：领域模型已有显式演示身份与加载操作契约；IndexedDB v4 已持久回填旧事件身份，低敏 `appPreferences` 使用单例存储和运行时校验，显式全量删除会清理偏好、中断操作与临时文件且单事件删除不清偏好。Task 3 及之后的公开演示资产、首次引导、隐私/关于页、提示式更新、生产安全头与部署资产尚未开始。
...
- M4 Public Demo and Deployment：`IN PROGRESS`；readiness 为 `APPROVED / READY`。Task 1–2 已验收，生产真实环境验证和部署尚未开始。
...
Next Action：M4 Task 3 — Publish and Validate the Fictional Public Demo Fixture。Task 2 已通过真实 Chromium v3→v4 持久回填、偏好往返/非法记录、删除回归、Web 测试与 typecheck，并完成独立 Review repair。
...
- `deleteAllLocalData` 已具备确定性服务与测试，但尚无用户可达入口；在 M4 公开信息/交互 Task 中必须接入或明确由现有事件删除流程覆盖，不能把不可达服务当作公开验收证据。

## Blockers

- M4 Task 3 当前没有未解决的设计、计划或分支 readiness blocker。
...
- Task 2 acceptance commit：包含本次 `PLAN.md` 更新并使用提交信息 `feat: migrate M4 local preferences`；未 push、未部署。
- 当前自动化证据：Task 1 目标与受影响回归 11 个 Vitest 文件、71 个测试通过；Task 2 Web 测试 97/97、Chromium 迁移/偏好/删除 9/9；fixture validator、typecheck 与 forbidden-content 门禁通过。
